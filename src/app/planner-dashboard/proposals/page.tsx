...
export default function ProposalsPage() {
  ...
  const { data: invitations, isLoading: isLoadingInvitations } =
    useCollection<TeamMemberInvitation>(pendingInvitationsQuery);
  ...
  const { data: acceptedGigs, isLoading: isLoadingGigs } =
    useCollection<AcceptedGig>(acceptedGigsQuery);
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            You have {invitations?.length || 0} pending invitations.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              >
                <div>
                  <p className="font-semibold">
                    {invitation.eventName || 'An Event'}
                  </p>
                  <p className="text-sm text-muted-foreground flex items-center gap-2">
                    <Calendar className="h-4 w-4" />
                    {invitation.eventDate
                      ? format(invitation.eventDate.toDate(), 'PPP')
                      : 'Date TBD'}
                  </p>
                </div>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          setClashWarning({ ...clashWarning, show: open })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>`{clashWarning.clashingEvent?.eventName}`</strong> on
              this date. Are you sure you want to accept this new event?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
