...
export default function FindPlannerPage() {
  ...
  const filteredPlanners = useMemo(() => {
    ...
    return allPlanners.filter(planner => {
      const displayName = planner.name || `${planner.firstName || ''} ${planner.lastName || ''}`.trim() || 'Professional Planner';
      const nameMatch = debouncedSearchTerm ? displayName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) : true;
