        ...
        <div className="max-w-2xl mx-auto">
             <div className="flex justify-between items-center mb-4">
                 <Button variant="outline" asChild>
                    <Link href={`/owner/stationery-hub/program-preview/${eventId}`}>
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Back to Program
                    </Link>
                </Button>
                <Button asChild>
                    <Link href={`/owner/stationery-hub/thank-you-notes/${eventId}`}>
...
export default function MenuPreviewPage({ params }: { params: Promise<{ eventId: string }> }) {
    ...
    return (
        <Suspense fallback={<div className="flex justify-center items-center h-full"><Loader2 className="h-8 w-8 animate-spin"/></div>}>
            <MenuPreviewPageContent eventId={resolvedParams.eventId} />
        </Suspense>
    );
}
