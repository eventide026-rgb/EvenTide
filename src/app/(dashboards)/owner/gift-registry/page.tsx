...
export default function GiftRegistryPage() {
  ...
  const { data: gifts, isLoading: isLoadingGifts } = useCollection<GiftType>(giftsQuery);
                                        ...
                                        {gifts && gifts.length > 0 ? gifts.map(gift => (
                                            <TableRow key={gift.id}>
                                                <TableCell className="font-medium flex items-center gap-4">
                                                    {gift.imageUrl && <div className="relative w-10 h-10 overflow-hidden rounded-md"><Image src={gift.imageUrl} alt={gift.name} fill className="object-cover" /></div>}
                                                    {gift.name}
                                                </TableCell>
                    ...
                    <CardContent>
                        <Form {...giftForm}>
                            <form onSubmit={giftForm.handleSubmit(handleAddGift)} className="space-y-4">
                                ...
                                <Button type="submit" className="w-full" disabled={giftForm.formState.isSubmitting}>
                                    {giftForm.formState.isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CirclePlus className="mr-2 h-4 w-4" />}
                                    Add to Registry
                                </Button>
                            </form>
                        </Form>
                    ...
                    <CardContent>
                         <Form {...bankForm}>
                            <form onSubmit={bankForm.handleSubmit(handleUpdateBankDetails)} className="space-y-4">
                                ...
                                <Button type="submit" className="w-full" disabled={bankForm.formState.isSubmitting}>
